...
- `-C <file.toml>` specify a configuration file for an architecture. The `configs`
  subdirectory of this repository contains various example
  configurations.
...
- `-I <register = value>` set the initial value of a register at the beginning of time,
  before any Sail initialization code occurs

- `-R <register = value>` set a register value after Sail initialization occurs.
...
- `-D <flags>` set debugging flags. For example, `-D f` will print information
  ...
  exection. The various options are:
...
- `--debug-id` Sometimes the `--probe` flag will display identifers as
...
- `mark-reg <registers> <string>` An event that can be used to tag
  ...
  registers simultanenously if needed. Currently this information is
...
The `-d` option changes the behavior of the command to instead print
...
![Message passing example](doc/LB.png?raw=true)
...
in the `web` subdirectory for how this is done. Note that while we try
...
branch. In the second version, both branches of the if are evaulated
